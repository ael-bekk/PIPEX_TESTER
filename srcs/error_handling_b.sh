...
#    Updated: 2022/02/17 15:42:12 by ael-bekk         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
sleep 0.7
	...
	printf "${WHITE}\tcatds /dev/random | ls | wc -l                                  : ${RESET}"
	...
	find_smae_err=`cat my_error | grep -a "no such file or directory\|No such file or directory" | wc -l`
	find_smae_err2=`cat my_error | grep -a "command not found" | wc -l`
	ls > outfile2
	different=`diff outfile outfile2`
	...
	printf "${WHITE}\tcsfat | wc -cl  | wasdc   |  grep 0  | so/rt | cat              : ${RESET}"
	...
	printf "${WHITE}\tsleep 12 | sleep | 7 | sleep 8 | sleep 4 | sleep 5 | sleep 2    : ${RESET}"
	SECONDS=0;
